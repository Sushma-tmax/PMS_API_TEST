...
export default function Timeline2(props: any) {
  ...
  const { data: caldata } = useAppraisalCalenderFiltersQuery(
    "?status=active&limit=1&populate=calendar"
  );
  //status
  ...
  const { data: allemployeeDatapending, isLoading } =
    ...
    useGetEmployeeByFilterQuery(
      `?select=appraisal.appraiser_status&appraisal.appraiser_status=pending&limit=600`
    );

  const { data: allemployeeDatadraft } =
    // useGetAllAppraiserStatusQuery("draft");
    useGetEmployeeByFilterQuery(
      `?select=appraisal.appraiser_status&appraisal.appraiser_status=draft&limit=600`
    );

  const { data: allemployeeDatanormalizerRejected } =
    // useGetAllAppraiserStatusQuery("normalizer-rejected");
    useGetEmployeeByFilterQuery(
      `?select=appraisal.appraiser_status&appraisal.appraiser_status=normalizer-rejected&limit=600`
    );

  const { data: allemployeeDatareviewerRejected } =
    // useGetAllAppraiserStatusQuery("reviewer-rejected");
    useGetEmployeeByFilterQuery(
      `?select=appraisal.appraiser_status&appraisal.appraiser_status=reviewer-rejected&limit=600`
    );
  ...
  const { data: allrevieweremployeeDatapending } = useGetEmployeeByFilterQuery(
    `?select=reviewer.reviewer_status&reviewer.reviewer_status=pending&limit=600`
  );
  const { data: allrevieweremployeeDatadraft } =
    // useGetAllReviewerStatusQuery("draft");
    useGetEmployeeByFilterQuery(
      `?select=reviewer.reviewer_status&reviewer.reviewer_status=draft&limit=600`
    );
  const { data: allrevieweremployeeDatanormalizerRejected } =
    // useGetAllReviewerStatusQuery("normalizer-rejected");
    useGetEmployeeByFilterQuery(
      `?select=reviewer.reviewer_status&reviewer.reviewer_status=normalizer-rejected&limit=600`
    );
  const { data: allrevieweremployeeDatareviewerRejected } =
    // useGetAllReviewerStatusQuery("reviewer-rejected");
    useGetEmployeeByFilterQuery(
      `?select=reviewer.reviewer_status&reviewer.reviewer_status=reviewer-rejected&limit=600`
    );

  const { data: allnormalizeremployeeDatapending } =
    // useGetAllNormalizerStatusQuery("pending");
    useGetEmployeeByFilterQuery(
      `?select=normalizer.normalizer_status&normalizer.normalizer_status=pending&limit=800`
    );
  const { data: allnormalizeremployeeDatadraft } =
    // useGetAllNormalizerStatusQuery("draft");
    useGetEmployeeByFilterQuery(
      `?select=normalizer.normalizer_status&normalizer.normalizer_status=draft&limit=800`
    );
  const { data: allnormalizeremployeeDatanormalizerRejected } =
    // useGetAllNormalizerStatusQuery("normalizer-rejected");
    useGetEmployeeByFilterQuery(
      `?select=normalizer.normalizer_status&normalizer.normalizer_status=normalizer-rejected&limit=800`
    );

  const { data: allnormalizeremployeeDatareviewerRejected } =
    // useGetAllNormalizerStatusQuery("reviewer-rejected");
    useGetEmployeeByFilterQuery(
      `?select=normalizer.normalizer_status&normalizer.normalizer_status=reviewer-rejected&limit=800`
    );
  ...
  React.useEffect(() => {
    if (
      ...
      const normalizerpendingEmployees = allnormalizeremployeeDatapending?.data;
