...
import { Container, Box, FormControlLabel, MenuItem, IconButton, FormControl } from "@mui/material";
  ...
  useEffect(() => {
    const mapped = users &&
      users?.filter((j: any) => {
        ...
        // columnHeadersDisplay?.filter((j: any) => {
        ?.map((j: any, emp: any, employee: any) => {
          // console.log(emp,"emp")
          // console.log(j,"j")
          console.log(j?.grade, "code")
          return {
            // {columnHeadersDisplay?.Ecode} && {Ecode :j?.employee_code}
            // ...(columnHeadersDisplay.["Ecode"]) && {Ecode: j?.employee_code}
            // ...(columnHeadersDisplay["Ecode"]) && { ECode: j.employee_code },
            // ...(columnHeadersDisplay["Ename"]) && { EmployeeName: j.legal_full_name },
            // ...(columnHeadersDisplay["Eposition"]) && { Position: j.position_long_description },
            // ...(columnHeadersDisplay["EGrade"]) && { Grade: j.grade },
            // ...(columnHeadersDisplay["Ependingaction"]) && { PendingAction: getPAStatus(j) },
            // ...(columnHeadersDisplay["Apprating"]) && {
            //   PendingAction: j.appraisal?.appraiser_rating == 0 ? (
            //     <p> - </p>
            //   ) : (
            ...
            //   PendingAction: (j.reviewer.reviewer_rating == 0 || j.reviewer.reviewer_status == "draft") ? (
            //     <p> - </p>
            //   ) : (
            ...
            //   PendingAction:(j?.normalizer?.normalizer_rating == 0 || j.normalizer.normalizer_status == "draft") ? (
            //     <p> - </p>
            //   ) : (
            //     j?.normalizer?.normalizer_rating
            //   )
            // },
            // ...(columnHeadersDisplay["Status"]) && { Status :getStatus(j.appraisal.status) },
            // // ...(columnHeadersDisplay["ViewPA"]) && { ViewPa :<img src={Eye} alt="icon" /> },
            // ...(columnHeadersDisplay["FirstName"]) && { FirstName : j?.first_name },
            // ...(columnHeadersDisplay["SupervisoryRole"]) && {supervisoryRole: j.isSupervisor != undefined
            //   ? "SP"
            //   : "N-SP" },
            //   ...(columnHeadersDisplay["Function"]) && { Function: ""},
            //   ...(columnHeadersDisplay["ServiceReferenceDate"]) && { ServiceReferenceDate:j?.service_reference_date},
            //   ...(columnHeadersDisplay["PositionCode"]) && { PositionCode:j?.position_code },
            //   ...(columnHeadersDisplay["division"]) && { Division:j?.division },
            //   ...(columnHeadersDisplay["Section"]) && { Section:j?.section },
            //   ...(columnHeadersDisplay["SubSection"]) &&  {Subsection : j["sub section"]},
            //   ...(columnHeadersDisplay["ManagerCode"]) && { Section:j?.manager_code },
            //   ...(columnHeadersDisplay["ManagerName"]) && { Section:j?.ManagerName },
            //   ...(columnHeadersDisplay["ManagerPosition"]) && { Section:j?.manager_position },
            //   ...(columnHeadersDisplay["WorkLocation"]) && { Section:j?.work_location },
            //   ...(columnHeadersDisplay["GradeSet"]) && { Section:j?.grade_set },
            //   ...(columnHeadersDisplay["JobCode"]) && { Section:j?.job_code },
            //   ...(columnHeadersDisplay["JobTitle"]) && { Section:j?.job_title },
            //   ...(columnHeadersDisplay["JobLevel"]) && { Section:j?.job_level },


            // WorkLocation GradeSet JobTitle
            // Status ViewPA ServiceReferenceDate PositionCode division Section ManagerCode ManagerName ManagerPosition
    ...
    // }, [users, employeeCode, employeeName, empgrades, positions, enteredName])
  })
  //   console.log(columnHeadersDisplay?.Ecode,"columnHeadersDisplay")

  //   let Exceldata = {}

  // Object.keys(columnHeadersDisplay).forEach((item:any)=>{
  // // console.log(columnHeadersDisplay,"columnHeadersDisplay")
  // if(columnHeadersDisplay[item]){

  //   Exceldata = {... item = ["Ecode"]}
  // }
  //     // setfilData(Exceldata)
