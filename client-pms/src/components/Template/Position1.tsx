...
export default function Position1(props: any) {
  const { ViewTemplateData, onDelete, checkedTemplatesID, name, setnavPrompt, refetch, MappedTemplates, fetchMapped, year, showemptyVal, setshowemptyVal,settriggerTheNav1,triggerTheNav, settriggerTheNav } =
    props;
  const { id } = useParams();
  console.log(setnavPrompt, "setnavPrompt");
  const { data, isLoading, error } = useGetEmployeeQuery("all");
  const { data: appraisalCalData, refetch: fetchAppraisalCal } = useGetAppraisalCalenderQuery("");
  const { data: unmappedCalData, refetch: fetchUNMapped, isLoading: isDataLoading } = useGetEmployeeUnmappedQuery(year)
  ...
  const [createMappedPositions, { isSuccess: isSuccessMappedPosition }] =
    useAddEmpolyeAppraisalCalenderMutation();
  const [removeMappedPositions, { isSuccess: isSuccessMappedPositions }] =
    useRemoveEmpolyeAppraisalCalenderMutation();
  ...
  useEffect(() => {
    let filteredUsersID = unmappedEmpDatum?.filter((j: any) => {
      if (employeeNameOther === "None" || employeeNameOther === "") {
        return j;
      } else {
        return j?.legal_full_name
...
// console.log(uniqueAges,"uniqueAges")
    // let unique = grades.filter((item: any, i: any, ar: any) => ar.indexOf(item) === i);
  ...
  }, [tabValues, activeTemplate, empsections, positions, empgrades, employeeCode, searchName2, employeeName])
  ...
  useEffect(() => {
    const mapped = activeTemplate?.filter((j: any) => {
      ...
      .map((j: any, emp: any, employee: any) => {
        ...
        return {
          EmployeeCode: j?.name?.employee_code,
    ...
    setfilData(unmapped);
    // }
  }, [tabValues, unmappedEmpDatum, employeeCodeOther, empsectionsother, employeeNameOther, empgradesother, positionsother, sNSOther, searchName2])
  ...
  useEffect(() => {
    ...
    console.log(mapped, "mapped")
  }, [tabValues, activeTemplate, employeeCode, empsections, employeeName, empgrades, positions, sNS, searchName2])
                          ...
                          <Stack direction="row">
                            ...
                            >
                              ...
                              {/* {
                                ...
                                {console.log(
  unmappedEmpDatum.filter((q, idx) => 
    cats.indexOf(q) === idx)
);}} */}


                              {/* { const Grade1= unmappedEmpDatum.map(q => q.grade);
                               Grade1.filter((q, idx) =>
                               Grade1.indexOf(q) === idx)
                                    } */}
                              {unmappedEmpDatum
            ...
            >
              ...
              <DialogContent>
                ...
                >
                  {tabValues !== 1 && (
                    <div>
                      Would you like to unmap selected employees from the template?
                    </div>
                  )}
                  {tabValues == 1 && (
                    <div>
                ...
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    if (tabValues === 1) {
                      ...
                      selectOneError(checkboxIdHandlerMapped(checkboxHandlerMapped(activeTemplate)));
                      onSubmitMapped({
                        position: checkboxIdHandlerMapped(checkboxHandlerMapped(activeTemplate)),
                      });
                      setsaveTrigger(true);
                      settriggerTheNav(false);
                    }
