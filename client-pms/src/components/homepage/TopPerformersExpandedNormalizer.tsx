...
const { data: employeeData } = useGetEmployeeByFilterQuery(
  ...
  useEffect(()=>{
    ...
    const mapped =topPerformerEmployees()?.filter((f:any) => f?.normalizer?.normalizer_rating >= 4).sort((a:any,b:any) => b?.normalizer?.normalizer_rating - a?.normalizer?.normalizer_rating)
    ...
    ?.map((j:any) => {
      ...
      Rating : j?.normalizer?.normalizer_rating
      };
    }
    )
    setfilData(mapped)
    ...
    >
          ...
          <Scroll>
           <Scrollbar style={{ width: "100%", height: "calc(100vh - 260px)" }}> 
            <Table size="small" aria-label="simple table">
              ...
              <TableBody>
              {topPerformerEmployees()?.filter((f:any) => f?.normalizer?.normalizer_rating >= 4).sort((a:any,b:any) => b?.normalizer?.normalizer_rating - a?.normalizer?.normalizer_rating)
       ?.slice(
        page * rowsPerPage,
     ...
     return(
                ...
                >
                  ...
                  >
                    {i?.normalizer?.normalizer_rating}
                  </TableCell>
                </TableRow>)})}
              </TableBody>
            </Table>
