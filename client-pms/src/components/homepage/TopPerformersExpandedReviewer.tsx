...
export default function TopPerformersExpandedReviewer(props: any) {
  ...
  useEffect(()=>{
    const mapped =topPerformerEmployees()?.filter((f:any) => f?.reviewer?.reviewer_rating >= 4).sort((a:any,b:any) => b?.reviewer?.reviewer_rating - a?.reviewer?.reviewer_rating)
    ?.map((j:any) => {
      return { 
        Ecode :j?.employee_code,
      Ename:j?.legal_full_name,
      Position :j?.position_long_description,
      Rating : j?.reviewer?.reviewer_rating
      };
    }
    )
    setfilData(mapped)
          ...
          <Scroll>
           <Scrollbar style={{ width: "100%", height: "calc(100vh - 260px)" }}> 
            <Table size="small" aria-label="simple table">
              ...
              <TableBody>
              {topPerformerEmployees()?.filter((f:any) => f?.reviewer?.reviewer_rating >= 4).sort((a:any,b:any) => b?.reviewer?.reviewer_rating - a?.reviewer?.reviewer_rating)
       ?.slice(
        page * rowsPerPage,
