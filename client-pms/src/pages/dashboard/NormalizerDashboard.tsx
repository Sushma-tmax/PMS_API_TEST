...
export default function NormalizerDashboard() {
    ...
    const { data: data2 } = useGetEmployeeByFilterQuery(`?select=appraisal.status,employee.employee_agree,employee.employee_status&calendar=${appCalId}&limit=800`)
    ...
    const TopPerformersOfNormalizer = employeeData?.data?.slice()?.filter((f:any) => f?.normalizer?.normalizer_rating >= 4)
