...
import { useToggleObjectiveTitleMutation } from "../../../service/Objective/ObjectiveTitle";
import { makeStyles } from '@mui/styles';
...
const ObjectiveTitle = (props: any) => {
  ...
  const [updateObjectiveDescription, { isError: errorObjectiveTitle1, isSuccess }] =
    useUpdateObjectiveDescriptionMutation();
  const [toggleObjectiveTitle] = useToggleObjectiveTitleMutation()
  useEffect(() => {
  ...
  const [toggleAlert, setToggleAlert] = useState<any>(false);
  ...
  const [disableObjectiveTypeSelect , setDisableObjectiveTypeSelect] = useState(false);
  ...
  const handleClose3 = () => {
    ...
    setDisableObjectiveTypeSelect(false)
  };
  ...
  const errorHandler = () => {
    if (
     ...
     // return (
        ...
        setObjectiveTypeSelect("")
        setnavPrompt(false)
        //dialog for no change
    //  );
   //   }else{
   //     setToggleAlert(true)
   //     setMessage("There are no changes to savess")
    //  }
    } else {
  ...
  const editHandler = (
    ...
    toggleObjectiveTitle({
      objectiveTitleId: objectiveTitleItem._id,
      objectiveTitle: title
    }).then((res:any) => {
      if (res.data.success && res?.data?.template?.status_template === "Completed") {
        setDisableObjectiveTypeSelect(true)
      }
    })
  };
  ...
  const handleactiveOuterCheckbox = (e: any, i: any) => {
    const { name, checked } = e.target;
    console.log(checked, 'checked')
    /* If check == true, return isTitleActive : checked and based on checked value it will be toggled on or toggled off,
         if check == false, check whether objective title is mapped with any template: 
         if mapped then cannot be toggled off , and if not mapped then it can be toggled on.  */
    if (checked) {
      ...
      toggleObjectiveTitle({
        ...
        if (res.data) {
          ...
          const tempUser = users.map((employee: any) => {
            return employee._id === name
              ? { ...employee, isTitleActive: true }
              : employee;
          });
          setUsers(tempUser);
          setactiveOuterCheckbox(true)
  ...
  return (
    <>
      ...
      >
                    ...
                    <Stack>
                      ...
                      >
                        <Select
                          ...
                          disabled= {disableObjectiveTypeSelect}
                          displayEmpty
                          renderValue={
                            objectiveTypeSelect !== ""
                              ? undefined
                                ...
                                control={
                                  <FormControlLabel control={
                                    <Switch
                                      ...
                                      // checked={activeCheckbox}
                                      onChange={(e: any) => { handleactiveOuterCheckbox(e, i) }}
        ...
        onClose={handleClickobjTitleClose}
      ...
      <Snackbar
        className={classes.customSnackbar}
        open={openobjTitledel}
        autoHideDuration={3000}
        onClose={handleClickobjTitledelClose}
        anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
      >
        <Alert
          className={classes.customAlert}
          ...
          icon={false}
        >
          <b>{description} has been deleted successfully.</b>
        </Alert>
      </Snackbar> 
        {/* <AlertDialogSuccess
        ...
        {/* alert for toggling objective title */}
        <AlertDialogSuccess
          isAlertOpen={toggleAlert}
          handleAlertClose={handleToggleAlertClose}
        >
          {message}
        </AlertDialogSuccess>
