...
import AlertDialogOkCancel from "../../UI/DialogOkCancel"
...
const EditCalendarMapping = (props: any) => {
  ...
  //   useGetAppraisalCalenderQuery("");
    const { data: appraisalCalendarData, isLoading: isDataLoading } = useGetCurrentAppraisalCalenderQuery ("")
...
//console.log(res[0]?.name,"selectedname")
  ...
  const checkboxHandler = (checkbox: any) => {
    if (checkbox) {
      ...
      console.log(res?.length,"ressssss")
      if(res?.length != 0){
          setnavPrompt(true)
        }else{
          setnavPrompt(false)
  ...
  const handleOnCheck = (e: any) => {
    ...
    // if (checkboxHandler(template)?.length == 0) {
    //   setnavPrompt(false);
    // }else {
    //   setnavPrompt(true);
    // }
    ...
    if (name === "allSelect") {
      ...
      if (tempUser?.filter((i:any) => i.isChecked)?.length > 0) {
        setnavPrompt(true)
      } 
      else {
        setnavPrompt(false)
  ...
  const handleCancelButton = () =>{
    ...
    setdialogActive(true)
    setDialogMsg("No changes are made to cancel.")
    }
  ...
  const onSubmit = () => {
    console.log(checkboxIdHandler(checkboxHandler(template))?.length, navPrompt,'lengthCheck')
    if (checkboxIdHandler(checkboxHandler(template))?.length == 0) { 
      setSuccess(false);   
      setOpen1(true);
    ...
    checkboxIdHandler(checkboxHandler(template))?.forEach((element: any) => {
      createAppraisalCalender({
        ...
        // if (navPrompt === true) {         
          ...
          settriggerTheNav(false);
          setSelect(false);
          let temp = template.map((item : any) => {
            return {
              ...item,
              isChecked : false
            }           
      ...
      >
        ...
        >
          ...
          <Grid container spacing={2}>
                      ...
                      <Table size="small" aria-label="simple table">
                        ...
                        {mappedTemplates &&
                          mappedTemplates.map((appCalendar: any) => {
                            console.log(appCalendar,"newwwww")
                            return (
                                      ...
                                      >
                                        ...
                                        >
                                         ...
                                         ( dayjs(
                                          ...
                                          !dayjs( appCalendar?.calendar?.start_date).isSame(dayjs(), 'day')) ? (
            ...
            <Grid sx={{ paddingTop: "5px !important",paddingLeft:"16px" }} xs={12} md={6}>
              <Paper
                elevation={3}
                sx={{
                  // paddingTop: "10px",
                  ...
                  >
                    <input
                                ...
                                checked={
                                 ...
                                 onChange={(e: any) => handleOnCheckbox(e)}
                                type="checkbox"
                                style={{
                                  height: "17px",
                                  width: "17px",
        ...
        <AlertDialogSuccess
          ...
          <AlertDialogOkCancel
            isAlertOpen={dialogActive}
            handleAlertClose={()=>{setdialogActive(false)}}
