...
const ViewCalendarMapping = (props: any) => {
  ...
  useEffect(() => {
    const Calendar = calenderData &&
      ...
      // }
     if(calenderData?.data?.length == 0 || calenderData?.data?.length == undefined){
        setemptyData(true)
      } else {
      setemptyData(false)
    }
    console.log(calenderData, emptyData,"newone")
  ...
  useEffect(() => {
    if (year && appraisalCalendarData) {
      ...
      /* Cannot Edit the calendar based on the condition : 
      if calendar start_date is before today's date, 
      if Calendar status is Live or Closed */
      ...
      console.log(editCalendarData, 'editCalendarData')
      setEdit(!editCalendarData.includes(false));
    }
  ...
  const dataAvailability =  template
  ?.filter((item: any) => {
    return (
      item?.weightage != "" &&
      item?.status_template == "Completed" &&
      item?.template_active_status == true
    );
  })
