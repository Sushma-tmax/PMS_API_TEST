...
import { useGetEmployeeQuery,useGetEmployeeByFilterQuery,
  useUpdateEmployeeAppraisalMutation, useRemovesMultipleEmployeeFromRoleExceptionMutation
 } from "../../../service";
...
export default function Leavers(props:any) {
  const classes = useStyles();
  const {values,checkedSwitch} = props;
  ...
  const SELECT_FOR_DASHBOARD = `employee_code,service_reference_date,legal_full_name,position_long_description,grade,first_name,,manager_code,manager_name,
  appraiser_name,normalizer_name,section,reviewer_name,employee_upload_flag,isLeavers,probation_status,function
  ,email,isRoleExceptionDraft,isLeaversDraft,appraiser_code_Draft,appraiser_name_Draft,reviewer_code_Draft,reviewer_name_Draft,normalizer_code_Draft,normalizer_name_Draft,isSupervisor,section,sub section,overall_rating,isSelected,division,manager_position,work_location,appraiser_code,reviewer_code,normalizer_code`
...
const handleChangeSelectGrade = (event: SelectChangeEvent<typeof GradeFilter>) => {
 ...
 const leevars =  users
 ?.filter((j: any) => {
   return  (j?.isLeavers || j?.isLeaversDraft)
 })
 console.log(leevars,"leevars")
//checkbox funcs
...
const appraiserNames = Array.from(new Set(data?.data?.filter((j: any) => {
  return  j?.isLeavers === true || j?.isLeaversDraft === true
    ...
    const f = users
      ...
      .filter((j: any) => {
        if (RevName === "None" || RevName === "") {
          ...
          return j?.reviewer_name
            ?.toLocaleLowerCase()
            ?.includes(RevName.toLocaleLowerCase());
        }
      })
      .filter((j: any) => {
        if (norName === "None" || norName === "") {
          ...
          return j?.normalizer_name
            ?.toLocaleLowerCase()
            ?.includes(norName.toLocaleLowerCase());
        }
      })
      .filter((j: any) => {
        if (empgrades === "None" || empgrades === "") {
...
employeeUpdate({
    ...
    setLaunchValDialogMSG("The selected employees were removed from Leavers")
});
}
...
?.filter((j: any) => {
  return  j?.employee_upload_flag  && (j?.isLeavers === true || j?.isLeaversDraft === true)
})?.filter((j: any) => {
    if (ProbationStatus === "None" || ProbationStatus === "") {
      return j;
    } else {
     ...
     <Box >
      ...
      {tableDataLength?.length > 0 && 
       ...
       marginTop="10px" marginBottom="10px"
          >
              {checkedSwitch === false && <Tooltip title="Delete">
                              <IconButton
            ...
            <Table size="small" aria-label="simple table" stickyHeader>
                        ...
                        <ListItemIcon>
                          <Checkbox
                           sx={{
                            ...
                            size="small"
                            style={{ padding: "0px", paddingLeft: "14px" , height: "0px",paddingRight: "10px"}}
             ...
             <TableBody>
                {data != undefined &&
                  ...
                  users
                  ?.filter((j: any) => {
                    return  j?.employee_upload_flag  && (j?.isLeavers || j?.isLeaversDraft)
                  })?.filter((j: any) => {
                      if (ProbationStatus === "None" || ProbationStatus === "") {
                        return j;
                      } else {
                    ...
                    > */}
                     {/* <Tooltip title="Edit">
                            ...
                            {/* <Tooltip title="Remove">
                              <IconButton
                                aria-label="CancelOutlinedIcon "
                                onClick={(e) => {handleRemove(emp?._id)}}
                 ...
                 <TableRow>
                   ...
                   colSpan={6}
                   align="center" 
                   style={{ fontWeight: 'bold',border:"none",color:"#808080",fontSize:"18px",display:"flex",width:"max-content",fontFamily:"arial"  }}
                   >
                     No data to display
                   </TableCell>
                 </TableRow>
