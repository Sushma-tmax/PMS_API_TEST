...
export default function Exclusions(props:any) {
  ...
  appraiser_name,normalizer_name,section,reviewer_name,isExcluded,probation_status,function,first_name,
  email,isSupervisor,service_reference_date,overall_rating,sub_section,sub_sectionsection,isSelected,sub section,division,manager_position,work_location,appraiser_code,reviewer_code,normalizer_code,employee_upload_flag`
...
//for appraiser ,reiewer and normalizer name options
 ...
 console.log(empEmployeeCode, "position");
  ...
  }, [users,positionsFilter,GradesFilter,searchNameForTabs]);
  //populating filter dropdown
...
setPage(0);
 ...
 setpositionsFilter(value);
  ...
  React.useEffect(() => {
    if (empsections === "None" || empsections === "" || empsections === "0") {
      setIcon13(false);
    } else {
      setIcon13(true);
    }
...
employeeUpdate({
    ...
    setLaunchValDialogMSG("Employee has been removed from Exclusion")
});
}
...
const handleMultiDelete = () =>{
  ...
  if(data.length > 0){
    removeFromRoleexception({
      ...
      res.error ? <> </> : 
      setLaunchValDialog(true);
      setLaunchValDialogMSG("The selected employees were removed from Exclusions")
  });
  }else{
    setLaunchValDialog(true);
...
?.filter((j: any) => {
  return  j.employee_upload_flag && j?.isExcluded === true 
})?.filter((j: any) => {
    if (ProbationStatus === "None" || ProbationStatus === "") {
      return j;
    } else {
...
No data to display
       ...
       {(tableDataLength?.length > 0 && tableDataFilterdLength > 0) && 
       <TablePagination
        sx={{
