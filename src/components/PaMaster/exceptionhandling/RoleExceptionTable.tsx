...
export default function RoleExceptionTable(props:any) {
  ...
  const {values,checkedSwitch, setCheckedSwitch} = props;
  ...
  const SELECT_FOR_DASHBOARD = `employee_code,legal_full_name,position_long_description,grade,function,manager_code,manager_name,
  appraiser_name,isRoleExceptionDraft,isLeaversDraft,appraiser_code_Draft,appraiser_name_Draft,reviewer_code_Draft,reviewer_name_Draft,normalizer_code_Draft,normalizer_name_Draft,section,normalizer_name,reviewer_name,isRoleException,master_appraiser_code,probation_status,first_name,
  master_appraiser_name,master_reviewer_code,master_reviewer_name,master_normalizer_code,master_normalizer_name,
  email,isSupervisor,section,overall_rating,service_reference_date,sub_section,sub section,isSelected,division,manager_position,work_location,appraiser_code,reviewer_code,normalizer_code,employee_upload_flag`
...
const handleChangeSelectGrade = (event: SelectChangeEvent<typeof GradeFilter>) => {
  ...
  const appraiserNames = Array.from(new Set(data?.data?.filter((j: any) => {
    return  j?.isRoleException === true || (j?.isRoleExceptionDraft === true &&  j?.isRoleException === false)
  })?.map((i:any) => i?.appraiser_name_Draft )))
  .sort((a: any, b: any) => a?.localeCompare(b))
  .filter((i:any) =>{
    return i != undefined
  })
  const ReviewerNames =  Array.from(new Set(data?.data?.filter((j: any) => {
    return  j?.isRoleException === true || j?.isRoleExceptionDraft === true
  })?.map((i:any) => i?.reviewer_name_Draft)))
 ?.sort((a: any, b: any) => a?.localeCompare(b))
 ?.filter((i:any) =>{
  return i != undefined
})
const NormalizerNames =  Array.from(new Set(data?.data?.filter((j: any) => {
  return  j?.isRoleException === true || j?.isRoleExceptionDraft === true
})?.map((i:any) => i?.normalizer_name_Draft)))
?.sort((a: any, b: any) => a?.localeCompare(b))
?.filter((i:any) =>{
  return i != undefined
})
...
setPage(0);
  ...
  React.useEffect(() => {
    ...
    else if (searchNameForTabs?.length > 0) {
      grades = users
        .slice()
        ?.sort(function (a: any, b: any) {
        ...
        ?.filter((i: any) => {
          if (searchNameForTabs.length > 0) {
            const searchTerms = searchNameForTabs.toLowerCase().split(" ");
  ...
  }, [users,positionsFilter,searchNameForTabs,GradesFilter]);
  //populating filter dropdown
  ...
  React.useEffect(() => {
    ...
    ?.filter((j: any) => {
      ...
      .filter((j: any) => {
        if (norName === "None" || norName === "") {
          ...
          return j?.normalizer_name_Draft
      ...
      .filter((j: any) => {
        if (empFunction === "None" || empFunction === "") {
          ...
          return j.function.toLocaleLowerCase() === empFunction?.toLocaleLowerCase()
        }
      })
      .filter((j: any) => {
        if (empPositionCode === "None" || empPositionCode === "") {
      ...
      .filter((j: any) => {
        ...
        if (searchNameForTabs === "") {
          ...
          (j.position_long_description !== undefined &&
            j.position_long_description
              .toLocaleLowerCase()
              .includes(searchNameForTabs.toLocaleLowerCase())) ||
              (j.appraiser_name_Draft !== undefined &&
                j.appraiser_name_Draft
                  .toLocaleLowerCase()
                  .includes(searchNameForTabs.toLocaleLowerCase())) ||
                  (j.probation_status !== undefined &&
                    j.probation_status
                      .toLocaleLowerCase()
                      .includes(searchNameForTabs.toLocaleLowerCase())) ||
                      (j.reviewer_name_Draft !== undefined &&
                        j.reviewer_name_Draft
                          .toLocaleLowerCase()
                          .includes(searchNameForTabs.toLocaleLowerCase())) ||
                          (j.function !== undefined &&
                            j.function
                              .toLocaleLowerCase()
                              .includes(searchNameForTabs.toLocaleLowerCase())) ||
                          (j.normalizer_name_Draft !== undefined &&
                            j.normalizer_name_Draft
                              .toLocaleLowerCase()
                              .includes(searchNameForTabs.toLocaleLowerCase())) ||
          (j.grade !== undefined &&
            j.grade
        ...
        // else if(searchNameForTabs !== ""){
        //   if (searchNameForTabs == "SP") {
        //     return j?.isSupervisor == true;
        //   }else if (searchNameForTabs == "N-SP") {
        //       return j?.isSupervisor != true;
        //     }
        // } 
      }).map((j: any, emp: any) => {
    ...
    positionsFilter,
    sectionsFilter,
    columnHeaders,heading1,heading2,heading5,heading6,
    heading8,heading9,heading4,heading19,heading10,heading7,
    heading11,heading12,heading13,heading14,heading15,heading16,
    heading17
  ]);
  //  React.useEffect(()=>{
  //   if(Normalizerdata){
    ...
    ?.filter((j: any) => {
      ...
      .filter((j: any) => {
        ...
        return {
          ...
          AppraiserName: j?.appraiser_name_Draft,
          ReviewerName: j?.reviewer_name_Draft,
          NormalizerName: j?.normalizer_name_Draft,
          Section: j.section,
          ServiceReferenceDate: j.service_reference_date,
          SubSection: j["sub section"],
          Division: j.division,
          ManagerPosition: j?.manager_position,
          WorkLocation: j?.work_location,
          AppraiserCode: j?.appraiser_code_Draft,
          ReviewerCode: j?.reviewer_code_Draft,
          NormalizerCode: j?.normalizer_code_Draft
        };
      });
...
?.filter((j: any) => {
  return  j.employee_upload_flag && (j?.isRoleException === true || j?.isRoleExceptionDraft === true)
})
  ?.filter((j: any) => {
    if (ProbationStatus === "None" || ProbationStatus === "") {
      return j;
...
if  (isLoading) {
  ...
  return (
    ...
    <Box >
     ...
     {tableDataLength?.length > 0 && 
        <Scroll>
          <CustomScrollbar style={{ minWidth: 650, height: "calc(100vh - 320px)" }}>
            <Table size="small" aria-label="simple table" stickyHeader>
                      ...
                      <Stack direction="row" alignItems="center" justifyContent="center" >
                        ...
                        >
                          ...
                          {users
                            ...
                            padding: "5px 15px 0px 15px",
                            //padding: "0px 10px 2px 17px",
                                  backgroundColor: ProbationStatus === name?.probation_status ? "#EAECED" : "",
              ...
              {tableDataFilterdLength > 0 ? (<TableBody>
                {data != undefined &&
                  // (rowsPerPage > 0
                ...
                <TableRow>
                  ...
                  colSpan={6}
                  align="center" 
                  style={{ fontWeight: 'bold',border:"none",color:"#808080",display:"flex",width:"max-content",fontSize:"18px",fontFamily:"arial"  }}
                  >
                    No data to display
                  </TableCell>
                </TableRow>
     ...
     {(tableDataLength?.length > 0 && tableDataFilterdLength > 0) && <TablePagination
        sx={{
