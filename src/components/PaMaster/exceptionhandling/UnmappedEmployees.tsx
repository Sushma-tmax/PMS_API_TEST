...
// import { useClearEmployeeMutation } from "../../service/employee/clearEmployeeMaster";
...
export default function UnmappedEmployees(props: any) {
  //prompt ------functions
  const [navPrompt, setnavPrompt] = React.useState(false);
  const location = useLocation();
  // const unmappedEmployeesData : any = location?.state?.unmappedEmployeesData;
  // console.log(unmappedEmployeesData,'unmappedEmployeesData')
  ...
  const { searchName, setSearchName, values, checkedSwitch, setCheckedSwitch } = props;
  ...
  const { data, isLoading, isFetching } = useGetEmployeeByFilterQuery(
  ...
  // const [clearEmployee] = useClearEmployeeMutation()
  ...
  // React.useEffect(() => {  
  //   if (unmappedEmployeesData) {
  //     setUsers(unmappedEmployeesData);    
  //   }
  // }, [unmappedEmployeesData]);

  console.log(users,'userCheck')
  //populating filter dropdown
  ...
  React.useEffect(() => {
    const Normalizerdata = users
      ...
      .map((j: any, emp: any) => {
        ...
        if (columnHeaders["EmployeeMappedStatus"] == true) exportData["Employee Mapped Status"] = j?.isMapped ? "Mapped" : "Unmapped"
        return exportData
        // return {
    ...
    revNameFilter,
    norNameFilter,
    columnHeaders, heading1, heading2,firstname ,heading5, heading6,
  ...
  const onSubmitRoleExceptions = (data: any) => {
    ...
    const probationdetails = data?.position?.filter((i: any) => {
      return i?.probationStatus === "inprobation"
    })
    ...
    if (probationdetails?.length > 0) {
      ...
      setLaunchValDialogMSG(`Employee ID ${ids.join(', ')} is on the probation period and can only be mapped to Exclusions or Leavers.`)
    } else {
  ...
  const onSubmitGradeException = (details: any) => {
    //check for probationStatus === "inProbation"
    //if so it cannot be added 
      ...
      employeeUpdateGradeException({
        ...
        if (res.error) {
          ...
          if (navPrompt) {
            setLaunchValDialog(true)
            // setLaunchValDialogMSG("Employees were moved to Grade Exception")
            setLaunchValDialogMSG("Selected employees were moved to Grade Exception")
            setnavPrompt(false)
          } else {
            setLaunchValDialog(true)
            setLaunchValDialogMSG("No employees were selected")
  ...
  const onSubmitCEORole = (data: any) => {
    ...
    if (probationdetails?.length > 0) {
      ...
      setLaunchValDialogYes(true);
      setLaunchValDialogMSG(`Employee ID ${ids.join(', ')} is on the probation period and can only be mapped to Exclusions or Leavers.`)
    } else if (data?.position[0]?.isRoleException) {
      setLaunchValDialog(true);
      setLaunchValDialogMSG("Employees were moved to the Role Exception")
  ...
  const handleClearEmployeeMaster = () => {
   ...
   let clearedEmployeeMasterData = data?.data?.map((item : any) => {
      ...
      //  clearEmployee({data: clearedEmployeeMasterData})     
    }
  }
  ...
  return (
    <>
              ...
              <Box sx={{ paddingLeft: "10px", paddingRight: "25px" }}>
                ...
                <FormGroup>
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox checked={heading1} name="Ecode" onChange={handleheading1} />
                    }
                    label="Ecode"
                  />
                  <FormControlLabel
                  ...
                  {/* <FormControlLabel
                    ...
                    control={
                      <Checkbox checked={heading3}  name="First Name" onChange={handleheading3} />
                    }
                    label="First Name"
                  /> */}
                  {/* <FormControlLabel
      ...
      {tableDataLength?.length > 0 && <Stack paddingTop="5px" paddingBottom="5px" direction="row" alignItems="center" justifyContent="space-between">
        <div>
          <FormGroup>
            <FormControlLabel
        ...
        >
          ...
          <Button variant="outlined"
            style={{ marginLeft: "15px", marginTop: "5px", marginRight: "15px" ,textTransform: 'none',fontSize: "15px",
            fontFamily: "Arial",
            borderColor: "#3E8CB5",
            color: "#3E8CB5",
            background: "transparent",}}
            onClick = {() => {handleClearEmployeeMaster()}}>             
           <Typography>Clear Employee Master</Typography> 
          </Button>

        </div>
      </Stack>}
      {tableDataLength?.length > 0 && <TableContainer component={Paper} sx={{ marginTop: 0 }}>
        <Scroll>
                  ...
                  >
                    <Stack direction="row" alignItems="center" justifyContent="center"  >
                      ...
                      >
                        ...
                        {users?.slice()?.sort(function (a: any, b: any) {
                ...
                {data != undefined &&
                  ...
                  users
                    ...
                    ?.map((emp: any, index: number) => {
                      console.log(emp,'emplppppppppppp')
                    return  (
                      <TableRow
                        key={index}
                        sx={{
      ...
      {tableDataLength?.length > 0 && <TablePagination
        sx={{
      ...
      {tableDataLength?.length > 0 && <Stack direction="row" alignItems="center" justifyContent="space-between">
