...
export default function GradeException(props:any) {
  ...
  const SELECT_FOR_DASHBOARD = `employee_code,legal_full_name,position_long_description,grade,,manager_code,manager_name,
  appraiser_name,section,normalizer_name,reviewer_name,isGradeException,probation_status,function,first_name,
  email,isSupervisor,section,service_reference_date,overall_rating,sub_section,sub section,isSelected,division,manager_position,work_location,appraiser_code,reviewer_code,normalizer_code,employee_upload_flag`
...
//checkbox funcs
  ...
  React.useEffect(() => {
    const thedata = users
    ?.filter((j: any) => {
      return  j?.isGradeException === true 
    })?.filter((j: any) => {
        if (positions === "None" || positions === "") {
          return j;
        } else {
...
employeeUpdate({
    ...
    setLaunchValDialogMSG("The employees were removed from Grade Exception")
});
}
...
const handleMultiDelete = () =>{
  ...
  if(data.length > 0){
    removeFromRoleexception({
      ...
      res.error ? <> </> : 
      setLaunchValDialog(true);
      setLaunchValDialogMSG("The employees were removed from Grade Exceptions")
  });
  }else{
    setLaunchValDialog(true);
...
?.filter((j: any) => {
  return j.employee_upload_flag && j?.isGradeException === true
})
  ?.filter((j: any) => {
    if (ProbationStatus === "None" || ProbationStatus === "") {
      return j;
