...
export default function FilteredExpandtable(props: any) {
  ...
  const { navigationFrom, EmployeeData, hideEmployeeRating, headersToDisplayWhenRenormalization, headersToDisplayWhenInMediation, headersToDisplayWhenCompleted } = props;
  ...
  useEffect(() => {
    ...
    //     })
    //     ?.filter((i: any) => {
    //       return !!sectionsFilter?.find((item2:any) => i?.section === item2)
    //     })
    //     ?.map((i: any) => {
    //       return i?.grade;
    //     });
  ...
  const handleExport = () => {
    ...
    const mapped = EmployeeData
      ...
      ?.map((j: any) => {
        ...
        //   j?.current_rating?.overall_rating == 0 || 
        //   j?.appraisal?.status !== "completed") ? "-" : j?.current_rating?.overall_rating
        if (columnHeaders["OverallRating"] === true) {
          exportData["Overall Rating"] = navigationFrom === "PreviousDashboard" ? (
        ...
        if (columnHeaders["rejectionStatus"] == true && (headersToDisplayWhenRenormalization || headersToDisplayWhenInMediation || headersToDisplayWhenCompleted)) exportData["Rejection Status"] = getRejectionstatus(j)
        return exportData
      });
    const a = [1]
    const Emptymapped = a.map((j: any) => {
      ...
      if (columnHeaders["rejectionStatus"] == true && (headersToDisplayWhenRenormalization || headersToDisplayWhenInMediation || headersToDisplayWhenCompleted)) exportData["Rejection Status"] = ""
      return exportData
    });
    //Export Funcs
  ...
  }, [EmployeeData, enteredName, sectionsFilter, GradesFilter, positionsFilter, GradeFilter, positionFilter])
                ...
                <Table size="small" aria-label="simple table">
                  ...
                  >
                    ...
                    >
                      ...
                      {(headersToDisplayWhenInMediation || headersToDisplayWhenRenormalization) &&
                        <TableCell
                          width="250px"
                          align="center"
                          sx={{
                  ...
                  {tableDataFilterdLength?.length > 0 ? (
                     ...
                     * is change in the rating that is ,if it is different from the normalized rating else it should be black*******************/
                      ...
                      * rating will be in blue else black***************************/
                                  ...
                                  {/* {(j?.appraisal?.status === "completed" && j?.employee?.employee_status === "rejected") ? (
                                    <span style={{ color: "#1976d2" }}>{j?.current?.normalizer_rating?.toFixed(2)}</span>
                                  ) : (j?.appraisal?.status === "completed") ? (<span>{j?.normalizer?.normalizer_rating?.toFixed(2)}</span>) :
                                    "-"
                                  } */}
                                </TableCell>}
                              ...
                              {/* {(!headersToDisplayWhenRenormalization && !headersToDisplayWhenInMediation && !headersToDisplayWhenCompleted) && 
                            <TableCell
                              align="center"
                              sx={{
                                fontFamily: "Arial",
