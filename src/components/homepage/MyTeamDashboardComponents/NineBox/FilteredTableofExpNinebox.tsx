...
export default function FilteredTableofExpNinebox(props: any) {
        ...
        <TableContainer>
          <Scroll>
            <CustomScrollbar style={{ width: "100%", height: "calc(100vh - 270px)" }}>
              ...
              ?.sort(
                (a: any, b: any) =>
                  b?.normalizer?.normalizer_rating - a?.normalizer?.normalizer_rating // Sort by pa_rating in descending order
                )
                ?.filter((item1: any) => {
                  if (employeeNamefilter.includes("None") || employeeNamefilter.length === 0) {
                    return item1;
