...
export default function TopPerformersExpandedNormalizer(props: any) {
  ...
  React.useEffect(() => {
    if (state?.activeCalenderName?._id !== undefined) {
      setappCalId(state?.activeCalenderName?._id)
    }
  ...
  //newly changed
  // const { data: employeeData } = useGetEmployeeByFilterQuery(
  //   `?calendar=${state?.appCalId}&limit=800&select=${SELECT_FOR_DASHBOARD}`
  // );
  ...
  //     ?.slice()?.sort((a: any, b: any) => b?.normalizer?.normalizer_rating - a?.normalizer?.normalizer_rating)?.slice(0, 5)
  // }
  ...
  const isAllpositionsFilter =
    positionArray?.length > 0 && positionFilter?.length === positionArray?.length;

  const handleChangeSelectPositions = (event: any) => {
    const value = event.target.value;
    if (value[value.length - 1] === "all") {
      console.log((positionFilter?.length === positionArray?.length ? [] : "select all"), "newwwwww")

      setpositionFilter(positionFilter?.length === positionArray?.length ? [] : positionArray);
      return;
    }
  ...
  gradesArrayM?.length > 0 && GradeFilter?.length === gradesArrayM?.length;
const newsection = gradesArrayM?.length == GradeFilter?.length
const handleChangeSelectGrades = (event: any) => {
  const value = event.target.value;
  if (value[value.length - 1] === "all") {
    console.log((GradeFilter?.length === gradesArrayM?.length ? [] : "select all"), "newwwwww")
    setGradeFilter(GradeFilter?.length === gradesArrayM?.length ? [] : gradesArrayM);
    return;
  }
  ...
  },[topPerformerEmployees,positionFilter,enteredName,GradeFilter])
  ...
  },[positionFilter,GradeFilter,sectionsFilter])
  ...
  const handleExport = () => {
    ...
    const mapped = topPerformerEmployees
      // ?.filter((f: any) => f?.normalizer?.normalizer_rating >= 4 && f?.appraisal?.status == "completed").sort((a: any, b: any) => b?.normalizer?.normalizer_rating - a?.normalizer?.normalizer_rating)
      ?.filter((item1: any) => {
  ...
  useEffect(()=>{
    ...
    // ?.filter((f: any) => f?.normalizer?.normalizer_rating >= 4)
    ?.filter((item1: any) => {
      if (positionFilter.includes("None") || positionFilter.length === 0) {
        return item1;
      } else {
  ...
  },[sectionsFilter,GradeFilter,positionFilter,enteredName])
  ...
  // ?.filter((f: any) => f?.normalizer?.normalizer_rating >= 4).sort((a: any, b: any) => b?.normalizer?.normalizer_rating - a?.normalizer?.normalizer_rating)
  ?.filter((item1: any) => {
    if (positionFilter.includes("None") || positionFilter.length === 0) {
      return item1;
    } else {
                                  ...
                                  <ListItemIcon>
                                    <Checkbox
                                      ...
                                      indeterminate={
                                        ...
                                        positionFilter?.length < positionArray?.length
                                      }
                                    />
                                  </ListItemIcon>
                              ...
                              <ListItemIcon>
                                <Checkbox
                                  ...
                                  indeterminate={
                                    ...
                                    GradeFilter?.length < gradesArrayM?.length
                                  }
                                />
                              </ListItemIcon>
                              <ListItemText
                                  ...
                                  ?.map((name: any, index: any) => (
                                    <MenuItem
                                      sx={{
                                        ...
                                        fontFamily: "Arial",
                                        padding: "0px 10px 2px 17px",
                                        backgroundColor: sectionFilter === name?.section ? "#EAECED" : "",
