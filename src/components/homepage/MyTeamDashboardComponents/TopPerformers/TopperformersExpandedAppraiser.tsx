...
let CalendarName =activecalendardata?.data[0]?.name
  ...
  // const SELECT_FOR_DASHBOARD = `appraisal.potential,appraiser_name,normalizer_name,reviewer_name,
  // service_reference_date,isSupervisor,email,function,employee_code,manager_code,legal_full_name,position_long_description,grade,appraisal.appraiser_rating,
  // reviewer.reviewer_rating,normalizer.normalizer_rating,overall_rating,appraisal.status,appraisal.appraiser_status,first_name,
  // reviewer.reviewer_status,normalizer.normalizer_status,reviewer.rejection_count,appraisal.objective_description,
  // manager_name,manager_position,talent_category,work_location,section,division,sub_section,reviewerIsDisabled`;
  ...
  useEffect(() => {
    ...
    const position = topPerformerEmployees
      ?.slice()
      ?.sort(function (a: any, b: any) {
 ...
 const [headingOfManagerPosition, setheadingOfManagerPosition] = React.useState(true);
  ...
  const handleExport = () => {
    ...
    let mapped = topPerformerEmployees
      // ?.filter((f: any) => f?.appraisal?.appraiser_rating >= 4 && f?.appraisal?.status == "completed")
      // .sort(
      //   (a: any, b: any) =>
      //     b?.appraisal?.appraiser_rating - a?.appraisal?.appraiser_rating
    ...
    console.log(Emptymapped, "Emptymapped")
  ...
  }, [enteredName, GradesFilter, positionsFilter, sectionsFilter, topPerformerEmployees])
