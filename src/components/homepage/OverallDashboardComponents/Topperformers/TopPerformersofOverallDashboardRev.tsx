...
export default function TopPerformersofOverallDashboardRev(props:any) {
  ...
  return (
    <div>
      ...
      {topPerformerEmployees && topPerformerEmployees?.filter((f:any) => f?.reviewer?.reviewer_rating >= 4 && f?.appraisal?.status == "completed")?.sort((a:any,b:any) => b?.reviewer?.reviewer_rating - a?.reviewer?.reviewer_rating)
      ?.map((i:any) => {
