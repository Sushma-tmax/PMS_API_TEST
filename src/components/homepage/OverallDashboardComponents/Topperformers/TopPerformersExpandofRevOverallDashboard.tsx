...
import { Container, Box, TablePagination, Breadcrumbs, FormGroup, InputAdornment, TextField, FormControl, MenuItem, ListItemText, ListItemIcon, Menu, } from "@mui/material";
...
export default function TopPerformersExpandofRevOverallDashboard(props: any) {
  ...
  console.log(topPerformerEmployeeso,"topPerformerEmployeeso")
  ...
  //     ?.sort(
  //       (a: any, b: any) =>
  //         b?.reviewer?.reviewer_rating - a?.reviewer?.reviewer_rating
  //     )?.slice(0, 5);
  // };
  ...
  },[topPerformerEmployeeso,positionsFilter,enteredName])
...
//section comps
const sectionArrays = topPerformerEmployeeso
// ?.filter((f: any) => f?.reviewer?.reviewer_rating >= 4 && f?.appraisal?.status == "completed")
?.slice()
...
console.log(sectionArrays,"sectionArrays")
 ...
 useEffect(()=>{
  ...
  useEffect(() => {
    const Paginate = topPerformerEmployeeso
      ...
      ?.sort(
        (a: any, b: any) =>
          b?.reviewer?.reviewer_rating -
          a?.reviewer?.reviewer_rating
      )?.filter((item1: any) => {
        if (positionsFilter.includes("None") || positionsFilter.length === 0) {
          return item1;
        } else {
  ...
  }, [enteredName, sectionsFilter, GradesFilter, positionsFilter, topPerformerEmployeeso])
