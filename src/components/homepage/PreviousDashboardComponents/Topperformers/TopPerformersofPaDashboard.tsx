...
export default function TopPerformersofPaDashboard(props:any) {
  const {topPerformerEmployees,navigationFrom,appCalId,valueOfActiveCalender} = props;
  const navigate = useNavigate();
  console.log(topPerformerEmployees?.length,"topPerformerEmployeeschild")
  ...
  const handleNavigationForTopPerformers = () =>{
    ...
    //   navigate("/TopPerformersExpandedViewofPA")
    //  }else if(navigationFrom === "Reviewer"){
    //   navigate("/TopPerformersExpandedViewofPA")
    //  }else if(navigationFrom === "Normalizer"){
     
    //   navigate("/TopPerformersExpandedViewofPA",{  state: { indexBasedValue:appCalId } }) 
    //  }
  ...
  return (
    <div>
      ...
      {topPerformerEmployees && topPerformerEmployees
       ?.slice(0, 10)
      // ?.filter((f:any) => f?.overall_rating >= 4).sort((a:any,b:any) => b?.overall_rating - a?.overall_rating)
      ?.map((i:any) => {
