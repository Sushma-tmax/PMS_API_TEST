...
import { Container, Box, TablePagination, Breadcrumbs, ListItemText, MenuItem, FormControl, ListItemIcon } from "@mui/material";
// import NBoxGrids from "./chartscomponents/nboxgrids";
import Typography from "@mui/material/Typography";
import Stack from "@mui/material/Stack";
import { styled } from "@mui/material/styles";
import { Button, IconButton, Drawer,InputAdornment, TextField, FormControlLabel, FormGroup } from "@mui/material";
...
export default function TopPerformersExpandedView(props: any) {
...
const CalendarName =state?.valueOfActiveCalender
  ...
  const SELECT_FOR_DASHBOARD = `appraisal.potential,appraiser_name,service_reference_date,isSupervisor,first_name,email,function,overall_rating,reviewer_name,normalizer_name,employee_code,legal_full_name,position_long_description,grade,appraisal.appraiser_rating,reviewer.reviewer_rating,normalizer.normalizer_rating,overall_rating,appraisal.status,appraisal.appraiser_status,reviewer.reviewer_status,normalizer.normalizer_status,reviewer.rejection_count,appraisal.objective_description,manager_code,manager_name,manager_position,work_location,section,division,sub_section,reviewerIsDisabled`
  const topPerformerEmployees = state?.topPerformerEmployees
  console.log(location,topPerformerEmployees, "location")
  // React.useEffect(() => {
  //   if (location.state.appCalId !== undefined) {
  //     setappCalId(location.state.appCalId)
  //   }
  ...
  const { data: employeeData } = useGetPreviousAppraisalEmployeeByFilterQuery(
    `?calendar=${location?.state}&limit=800&select=${SELECT_FOR_DASHBOARD}`
  );
  // const topPerformerEmployees = () => {
  //   return employeeData?.data
  //     ?.filter((f: any) => f?.appraisal?.status == "completed")
  //     ?.slice()?.sort((a: any, b: any) => b?.overall_rating - a?.overall_rating)?.slice(0, 5)
  // }
  ...
  React.useEffect(() => {
    let grades =topPerformerEmployees.slice()
  ...
  // console.log(GradesFilter,"GradesFilter")
   // search functionality based on position
    ...
    // sectionArray
  },[topPerformerEmployees,positionsFilter,enteredName,GradesFilter])
  // sectionFilter
  console.log(sectionArray,"sectionArray")
  const isAllsectionFilter =
  sectionArray?.length > 0 && sectionFilter?.length === sectionArray?.length;

const handleChangeSelectsections = (event: any) => {
  const value = event.target.value;
  if (value[value.length - 1] === "all") {
    console.log((sectionFilter?.length === sectionArray?.length ? [] : "select all"), "newwwwww")

    setSectionFilter(sectionFilter?.length === sectionArray?.length ? [] : sectionArray);
    return;
  }
  ...
  const handleExport = () => {
    ...
    const mapped = topPerformerEmployees
      ?.filter((f: any) => f?.overall_rating >= 4 && f?.appraisal?.status == "completed").sort((a: any, b: any) => b?.overall_rating - a?.overall_rating)
      ?.filter((item1: any) => {
      ...
      ?.map((j: any) => {
        ...
        if (columnHeaders["appraiserRating"] === true) {const appraiserRating = parseFloat(j?.normalized_overallRating);
  ...
  //         })
  //         ?.filter((i: any) => {
  //           return !!positionFilter?.find(item2 => i?.position_long_description === item2)
  //         })
  //         ?.map((i: any) => {
  //           return i?.grade;
  //         });
  ...
  // },[topPerformerEmployees,positionFilter])
  ...
  },[positionsFilter,GradesFilter,sectionFilter])
  useEffect(()=>{
    const Paginate =topPerformerEmployees?.filter((f: any) => f?.overall_rating >= 4)
    ?.filter((item1: any) => {
      if (positionsFilter.includes("None") || positionsFilter.length === 0) {
        return item1;
      } else {
  ...
  },[topPerformerEmployees,sectionFilter,GradesFilter,positionsFilter,enteredName])
  ...
  const tableDataFilterdLength =topPerformerEmployees?.filter((f: any) => f?.overall_rating >= 4).sort((a: any, b: any) => b?.overall_rating - a?.overall_rating)
  ?.slice(
    page * rowsPerPage,
            ...
            <Scroll>
              <CustomScrollbar style={{ width: "100%", height: "calc(100vh - 270px)" }}>
                <Table size="small" aria-label="simple table">
                            ...
                            <Stack direction="row">
                              ...
                              >
                               <MenuItem
                                ...
                                ?.filter((f: any) => f?.overall_rating >= 4)
                                  ?.slice()
                                  ?.sort(function (a: any, b: any) { return a.position_long_description?.localeCompare(b.position_long_description); })
                                  ?.filter((item: any, index: any, array: any) => array.map((data: any) => { return data.position_long_description }).indexOf(item.position_long_description) === index)
                                  ?.map((name: any, index: any) => (
                                  ...
                                  <ListItemIcon>
                                    <Checkbox
                                      ...
                                      indeterminate={
                                        ...
                                        sectionFilter?.length < sectionArray?.length
                                      }
                                    />
                                    </ListItemIcon>
                                    <ListItemText
                                    ...
                                    >
                                      <ListItemIcon>
                                        <Checkbox
                                          ...
                                          style={{ padding: "0px", paddingLeft: "14px", height: "0px" }}
                                          size="small"
                                          checked={sectionFilter.indexOf(name) > -1} />
                                      </ListItemIcon>
                                      <ListItemText
                                        primaryTypographyProps={{ fontSize: "13px", fontFamily: "arial", color: "#333333", paddingRight: "10px" }}
                                        primary={name} />
                         ...
                         {/* <FormControl sx={{ m: 0, width: 80, height: "0" }}>
                            <Stack direction="row">
                              ...
                              >
                                ...
                                {topPerformerEmployees()
                                  ?.slice()
                                  ?.sort(function (a: any, b: any) { return a?.appraisal?.potential?.localeCompare(b?.appraisal?.potential); })
                          ...
                          {/* <FormControl sx={{ m: 0, width: 80, height: "0" }}>
                            <Stack direction="row">
                              ...
                              <Select
                                ...
                                multiple
                                value={talentfilter}
                                onChange={handleChangeSelectTalentCategory}
                                renderValue={(selected) => selected.join(', ')}
                              >
                                <MenuItem
                                ...
                                {topPerformerEmployees()
                                  // ?.slice()
                                  // ?.sort(function (a: any, b: any) { return a?.talent_category.localeCompare(b?.talent_category); })
                                  ?.filter((item: any, index: any, array: any) => array?.map((data: any) => { return data?.talent_category }).indexOf(item?.appraisal?.potential) === index)
                                  ?.map((name: any, index: any) => (
                                    <MenuItem
                                      sx={{
                                        padding: "0px",
                                      ...
                                      key={name?.talent_category}
                                      value={name?.talent_category}
                                    >
                                      <Checkbox
                                        style={{ padding: "6px", paddingLeft: "12px" }}

                                        size="small"
                                        checked={talentfilter.indexOf(name?.talent_category) > -1} />
                                      <ListItemText
                                        primaryTypographyProps={{ fontSize: '0.9rem' }}
                                        primary={name?.talent_category} />
                                    </MenuItem>
                                  )
                                  )}
                              </Select>
                  ...
                  <TableBody>
                    {topPerformerEmployees?.filter((f: any) => f?.overall_rating >= 4)
                          ...
                          >
                            ...
                            <TableCell
                              ...
                              sx={{
                                ...
                                borderColor: "lightgrey",
                                fontSize: "14px",
                                color: (i?.appraisal?.status === "completed" && i?.employee?.employee_status === "rejected")?"#1976d2":"#33333",
                                wordBreak: "break-word",
                              }}
                            >
                              {i?.overall_rating?.toFixed(2)}
                            </TableCell>
                          </TableRow>)
                      })}
                  </TableBody>
