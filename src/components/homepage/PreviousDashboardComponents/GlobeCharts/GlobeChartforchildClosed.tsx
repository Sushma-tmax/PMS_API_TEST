...
export default function FilteredExpandtableClosed(props: any) {
  const classes = useStyles();
  const { EmployeeData, hideEmployeeRating, CalendarName,headersToDisplayWhenRenormalization, headersToDisplayWhenInMediation, headersToDisplayWhenCompleted } = props;
  ...
  const handleExport = () => {
    console.log("clickedmappedmapped")
    const mapped = EmployeeData
      ...
      ?.map((j: any) => {
        ...
        // if (columnHeaders["appraiserRating"] == true) exportData["Appraiser Rating"] = j?.normalized_overallRating?.toFixed(2)
    ...
    //Export Funcs
    console.log(mapped, "mappedmappedmapped")
  ...
  }, [EmployeeData, enteredName, GradesFilter, positionsFilter, GradeFilter, positionFilter])
  ...
  // positionsFilter  GradesFilter  FilterAltTwoToneIcon
  useEffect(() => {
    if (positionsFilter?.length == 0) {
      setIcon2(false);
    } else {
                    ...
                    <div>
                      ...
                      <FormControlLabel
                        ...
                        <FormControlLabel
                          ...
                          label="Status"
                        />
                        {/* {(headersToDisplayWhenRenormalization ||headersToDisplayWhenInMediation || headersToDisplayWhenCompleted) && <FormControlLabel
                          sx={{
                            "& .MuiFormControlLabel-label": {
                              fontSize: "14px",
                              color: "#333333",
                                  ...
                                  >
                                    ...
                                    <ListItemIcon>
                                      <Checkbox
                                        ...
                                        style={{ padding: "0px", paddingLeft: "14px", height: "0px" }} checked={positionsFilter.indexOf(option) > -1}
                                      />
                                    </ListItemIcon>
                    ...
                    <TableBody>
                      ...
                      {EmployeeData
                        ...
                        ?.filter((j: any) => {
                          ...
                          // const Overall_Rating =j?.normalized_overallRating?.toFixed(2)
                          // const app_rating =j?.appraiser_rating?.toFixed(2)
                          return (
                            <TableRow
                              sx={{
                          ...
                          {j?.employee_code}
                              ...
                              {(headersToDisplayWhenInMediation || headersToDisplayWhenRenormalization || headersToDisplayWhenCompleted) &&
                                ...
                                >
                                  {j?.normalized_overallRating ?  Number(j?.normalized_overallRating)?.toFixed(2) : Number(j?.appraiser_rating)?.toFixed(2)}
                                  {/* { j?.appraiser_rating && <span>{typeof j?.appraiser_rating === 'number' ? j?.appraiser_rating.toFixed(2) : '-'}</span> } */}
                                  {/* {j?.normalized_overallRating ? (<span>{typeof j?.normalized_overallRating != ""  ? j?.normalized_overallRating?.toFixed(2) : (typeof j?.appraiser_rating === 'string' ? Number(j.appraiser_rating).toFixed(2) : '-')}</span>
                                  ) : (
                                    <span> - </span>)} */}
                              ...
                              {(headersToDisplayWhenInMediation || headersToDisplayWhenRenormalization || headersToDisplayWhenCompleted) &&
                                ...
                                >
                                  ...
                                  {/* { j?.employee_rating && typeof j?.employee_rating === 'number' ? <span style={{ color: j?.appraisal?.status == "rejected" ||  j.employee.employee_status === "rejected" ? "red" : "#333333"}}>{ j?.employee_rating?.toFixed(2)}</span>  : <span> - </span>} */}
                                  {j?.employee_rating ? (
                         ...
                         * is change in the rating that is ,if it is different from the normalized rating else it should be black*******************/
                                ...
                                >
                                  ...
                                  {/* {typeof j?.normalized_overallRating === 'number' ? j?.normalized_overallRating?.toFixed(2) : "-"} */}
                                </TableCell>
                              }
                              {headersToDisplayWhenRenormalization &&
                                <TableCell
                                  ...
                                  sx={{
                                    ...
                                    borderColor: "lightgrey",
                                    fontSize: "14px",
                                    // color: (j?.reviewer_PA_rejected === true) ? "red" :
                                    // (j?.appraisal?.status == "rejected" && j?.reviewer_PA_accepted == true) ? "#1976d2" : "#333333",
                                    wordBreak: "break-word",
                                  }}
                                >
                       ...
                       * is change in the rating that is ,if it is different from the normalized rating else it should be black*******************/}
                                  {
                                    j?.reviewer_rating ?
                                      ...
                                      <span> - </span>
                                                                          ...
                                                                          {/* {typeof j?.reviewer_rating === 'number' ? j?.reviewer_rating?.toFixed(2) : '-'}</span> */}
                                </TableCell>
                              }
                              {(!headersToDisplayWhenRenormalization && !headersToDisplayWhenInMediation) &&
                                <TableCell
