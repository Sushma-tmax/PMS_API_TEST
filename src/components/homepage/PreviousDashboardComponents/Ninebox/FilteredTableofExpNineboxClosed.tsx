...
import { Button, IconButton,FormControlLabel,FormControl,MenuItem } from "@mui/material";
...
export default function FilteredTableofExpNineboxClosed(props: any) {
const classes = useStyles();
const {EmployeeData,hideEmployeeRating,AllEmployee,CalendarName} = props;
...
const handlefirstName = (event: React.ChangeEvent<HTMLInputElement>) => {
 ...
 }, [AllEmployee,positionsFilter,GradesFilter,enteredName])
 ...
 const handleChangeSelectEmployee = (event: SelectChangeEvent<typeof employeeNamefilter>) => {
  const {
    target: { value },
  } = event;
...
?.filter((j: any) => {
  ...
  ?.map((j: any) => {
    ...
    if (columnHeaders["Ecode"] == true) exportData["Ecode"] = j?.employee_code
        ...
        // if (columnHeaders["employeerating"] == true) exportData["Employee Rating "] = j?.employee_rating
        // if (columnHeaders["appraiserRating"] == true) exportData["Appraiser Rating"] = j?.appraiser_rating
        // if (columnHeaders["reviewerRating"] == true) exportData["Reviewer Rating"] = j?.reviewer_rating
        // if (columnHeaders["normalizerRating"] == true) exportData["HR Normalizer Rating"] = j?.normalizer_rating
        // if (columnHeaders["OverallRating"] == true) exportData["Overall Rating"] = j?.overall_rating
...
},[AllEmployee,enteredName,sectionfilter,GradeFilter,GradesFilter,positionsFilter,sectionsFilter])
                              ...
                              >
                                <ListItemIcon>
                                        <Checkbox
                                          ...
                                          style={{ padding: "0px", paddingLeft: "14px", height: "0px" }}
                                          size="small"
                                          checked={sectionsFilter.indexOf(name) > -1} />
                                      </ListItemIcon>
                                      <ListItemText
                        ...
                        {j?.legal_full_name}
                          ...
                          <TableCell
                            ...
                            sx={{
                              ...
                              borderColor: "lightgrey",
                              fontSize: "14px",
                              color: (j?.appraisal?.status === "completed" && j?.employee?.employee_status === "rejected")?"#1976d2":"#33333",
                              wordBreak: "break-word",
                            }}
                          >
                           {(
                                //   j?.overall_rating == 0 || j?.normalizer?.normalizer_status == "draft") ? (
                                //     <span> - </span>
                                // ) : (
