...
export default function GradeException(props:any) {
  ...
  appraiser_name,section,normalizer_name,reviewer_name,isGradeException,probation_status,first_name,
...
employeeUpdate({
    ...
    setLaunchValDialogMSG("Selected employees were removed from Grade Exception")
});
}
...
const handleMultiDelete = () =>{
  ...
  if(data?.length > 0){
    removeFromRoleexception({
      ...
      res.error ? <> </> : 
      setLaunchValDialog(true);
      setLaunchValDialogMSG("The selected employees were removed from Grade Exceptions")
  });
  }else{
    setLaunchValDialog(true);
