...
export default function ReviewerRating(props: any) {
  ...
  // @ts-ignore
  const { empData: employeeData, employee_id, reviewerOverallFeedComments, setReviewerOverallFeedComments, employeeDataIsFetching, refetchEmployeeData, disableTextAfterSubmission } = useReviewerContext();
  ...
  const showActionStatus = (j: any) => {
    if (employeeData?.data?.reviewer?.rejection_count == undefined ||
      employeeData?.data?.reviewer?.rejection_count == 0) {
                                  ...
                                  ((item: any) => (item?.rating_rejected == true && (item.rejection_reason !== "" && item.rejection_reason !== undefined))).length > 0 && (
                                    ...
                                    employeeData.data.appraisal.objective_description
                                      ...
                                      .map((k: any) => {
                                        ...
                                        {employeeData &&
                                          getAppraiserRejectionAttachments(j?.name?._id)?.length >
                                          0 && (employeeData?.data?.appraisal?.objective_description
                                            .filter(
                                              (i: any) =>
                                                i?.name?._id === j?.name?._id
                                            )
