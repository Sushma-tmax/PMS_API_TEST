...
export default function GridAutoFlow(props: any) {
  ...
  // const { data } = props;
  const { data, isLoading, refetch: dataFetch } = useGetTemplateQuery("");
  const { id } = useParams();
  //const id = '62b2eaa250b50c443ef9759a'
  const { data: singleTemplateData, refetch: singleTemplateDataFetch } = useGetSingleTemplateQuery(id);
  console.log(id, "id");
  ...
  const [activateCheckValidation, setActivateCheckValidation] = useState(false)  /* if sum !== 100 activate alert */
  ...
  useEffect(() => {
    ...
    if (idAlert) {
      seterrorMessage(" Please add objective type to the template.")
    }
    else if (textfeildError && textfeildError) {
      seterrorMessage(" The weightage value should be entered in Objective Group to be able to save the template.")
    }
    else if (sumError && error) {
      seterrorMessage("The sum of the Objective Group should be equal to 100%.");
    }
    else if (textfeildError1 && textfeildError1) {
      seterrorMessage("The weightage value should be entered in Objective Type to be able to save the template.")
    }
    else if (sumError1 && error1) {
      seterrorMessage(" The sum of the Objective Type should be equal to Objective Group.")
    }
    else if (textfeildError2 && textfeildError2) {
      seterrorMessage("  The weightage value should be entered in Objective Title to be able to save the template.")
    }
    else if (sumError2 && error2) {
      ...
      setOpen2(false)
      //seterrorMessage("Changes were successfully saved.")
      //snackbar
  ...
  useEffect(() => {
    if (objectiveType) {
      if (singleTemplateData?.template?.weightage?.objective_group?.length > 0) {
        let type = getObjectiveType(activeObjectiveGroup);
        setActiveObjectiveType(type);
        colorHandler(type);
        setActiveFocus2(0);
      }
  ...
  const validateObjectiveTitle = () => {
    ...
    console.log(checkResult, 'checksave')
  ...
  useEffect(() => {
    const sum = () => {
      ...
      if (r === 100) {
        setError(false);
        setSumError(false);
        setActivateCheckValidation(false);
        sumtotal();
        //  console.log('no error')
      } else {
        setError(true);
        setErrorHandler()
        setActivateCheckValidation(true)
        // console.log('error')
      }
    };
    // sum();
    ...
    const sumDescription = () => {
      ...
      return checkResult.includes(true) ? (setError2(true), setErrorHandler2(), setActivateCheckValidation(true)) : (setError2(false), setSumError2(false), setActivateCheckValidation(false));
  ...
  console.log(weightageHandler(objectiveGroup), weightageHandler(objectiveType), weightageHandler(objectiveDescription), "objectiveDescriptionobjectiveDescription")
  ...
  const saveHandler = () => {
    if (objectiveGroup && objectiveType && objectiveDescription) {
      ...
      /* Checking whether any field for group is empty or not */
      settextfeildError(() => {
        return weightageHandler(objectiveGroup);
      });

      /* Checking whether any field for type is empty or not */
      settextfeildError1(() => {
        return weightageHandler(objectiveType);
      });

      /* Checking whether any field for title is empty or not */
      settextfeildError2(() => {
        return weightageHandler(objectiveDescription);
      });
      ...
      if (
        ...
        !textfeildError1 &&
        !textfeildError2 &&
        activateCheckValidation    /* this is to check if sum is !== 100 for group, type and title */
        //  &&
        // !error &&
        // !error1 &&
        // !error2
        ...
        setOpen2(true)
        // validateObjectiveTitle();  commented by Sushma on 6/7/2023
        // .then((res: any) => {
        ...
        setActivateCheckValidation(false)
        validateObjectiveTitle()    /* here it is checking if sum of title value == 100 and saving the template */
        setOpen2(true)
      }
    }
  };

  console.log(activateCheckValidation, 'activateCheckValidation')
  const SaveTemplate = () => {
