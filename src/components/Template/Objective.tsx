...
export default function TemplateObjectives(props: any) {
  ...
  const [descriptionEmptyTextError, setDescriptionEmptyTextError] = useState(false)
...
console.log(singleTemplateData?.template?.name,"singleTemplateData")
  ...
  const { id } = useParams();
  const { data: singleTemplate, isLoading: load, refetch: fetchCancel } =
  ...
  const objectiveGroupIdHandler = (objectiveType: any) => {
    const uniqueGroupIds = new Set();
    const filteredGroup = objectiveType
      .filter((i: any) => {
        return i.isChecked === true && i?.objective_group?.name;
      })
      .map((item: any) => {
        console.log(item.objective_group,"itemmmmmm");
        /* function to check objective group to be unique */
        const groupId = item.objective_group?.name._id;
    ...
    console.log(filteredGroup, "return filteredGroup;");
  ...
  const objectiveGroupIds = objectiveGroupIdHandler(objectiveType);
  useEffect(() => {
    console.log(objectiveGroupIds[0]?.value,"neeeeeeeeeeeeeeeee");
    console.log(
      // objectiveGroupIds.length > 0 ? objectiveGroupIds[0].value : undefined,
      objectiveGroupIdHandler(objectiveType),
      addNametoId(objectiveType)[0]?.value,
      addNametoIdDesc(activeDesc)[0]?.value,
  ...
  const createHandler = (e: any) => {
    ...
    if (title === "") {
      ...
      return setDescriptionEmptyTextError(true)
    }
    if (id) {
      return (
        settextfeildError(false),
        setDescriptionEmptyTextError(false),
        editTemplateHandler({
          // name: title,
  ...
  const handleDialogOpen = () => {
    ...
    if (activeDesc) {
      if (res.length > 0) {
        ...
        setOpen2(false);
        setObjChecked(false);
        if (id && objectiveGroupIdHandler(objectiveType)[0]?.value != "" &&  addNametoId(objectiveType)[0]?.value != "" &&
        addNametoIdDesc(activeDesc)[0]?.value != "") {
          setIsOpenAlert(true);
  ...
  const handleDialogNo = (e:any) => {
    setOpen(false);
    if(singleTemplateData?.template?.name == undefined || singleTemplateData?.template?.name == "" ){
      setIsOpen(true);
    }else{
    createHandlerAlert(e);
    }
  ...
  useEffect(() => {
    if (singleTemplate) {
      console.log(
        singleTemplate.template.weightage.objective_type, singleTemplate.template.description,
        "ssssssssss"
      );
      setTitle(singleTemplate.template.name);
      setDescription(singleTemplate.template.description);
    }
  }, [singleTemplate]);
  ...
  return (
    <>
                          ...
                          <DialogContent>
                            ...
                            >
                              ...
                              {hideAlert && error1 === true && (
                                <div>Please change the template name. The template name already exists.</div>
                              )}
                            ...
                            <Stack>
                              ...
                              <Tf3>
                                <TextField
                                  ...
                                  error={!description && descriptionEmptyTextError}
                                  helperText={
                                    !description && descriptionEmptyTextError
                        ...
                        >
                          <Button
                            ...
                            onClick={(e) => {
                              createHandler(e);
                              // errorHandler()
                              //chech wheather the navprompt is turned of.
                            }}
      ...
      <Snackbar
        ...
        onClose={handleClickLaunchClose}
