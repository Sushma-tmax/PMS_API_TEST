...
const ViewMappedEmployee = (props: any) => {
  ...
  console.log(calenderData,"calenderDataFromquery")
...
//for hiding unmapped
  const [hideUnmapped,sethideUnmapped] = useState<any>(true)

  // click on template move into tabvalue 0
  ...
  console.log(year,"calenderDataFromquery")
  ...
  useEffect(() => {
    if (year && appraisalCalData) {
      ...
      //   if (i?.status == "active" || 
      //   i.calendar?.status == "Live" ||
      //   i.calendar?.status == "Closed" ||
      //   ( dayjs(
      //     i?.calendar?.start_date
      ...
      //   }
      // });
      let editCalendarData = calenderData?.data?.filter((calendar: any) => calendar?._id == year)
        ?.map((calendarItem: any) => {
          if ((!dayjs(calendarItem.start_date).isSame(dayjs(), 'day') && dayjs(calendarItem.start_date).isBefore(dayjs())) || calendarItem.status == "Live" || calendarItem.status == "Closed") {
            return false
          } else {
            return true
          }
        })
      setDisplayEdit(!editCalendarData.includes(false))
    }
  ...
  useEffect(() => {
    if (templateID) {
      console.log(templateID,'defValue')
      ...
      //   })
        console.log(defValue,templateID,"defValue")
        if(defValue !== undefined){ setYear(defValue[0]?._id)}
    }
    const defValue = calenderData &&
      activeAppId !== undefined &&
      calenderData?.data?.map((j:any)=>{
 ...
 useEffect(() => {
  if (MappedTemplates) {
    ...
    if (templateIndex >= 0) {
      ...
      navigate(`${VIEW_MAPPED_EMPLOYEE}/${temp?._id}`);
    }}
  }, [name])
...
//for hiding unmapped
const hideunmappForClosedCalender = () =>{
  const calenders = calenderData?.data?.filter((i:any)=>{
      return i.status === "Closed"
  }).map((i:any)=>{
    return i._id
  })
  console.log(calenders,"calenderscalenders")
  let data = calenders.includes(year)
  console.log(data,"calenderscalenders")
  if(calenders.includes(year)){
