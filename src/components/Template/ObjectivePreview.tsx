...
import { CREATE_TEMPLATE_1, EDIT_TEMPLATE_1, MASTER_NAV } from "../../constants/routes/Routing";
...
export default function ObjectivePreview() {
  ...
  let fromTemplate = location.state?.name;
  let Changes = location?.state?.changes
  console.log(fromTemplate,location,'fromTemplate')
  ...
  return (
    ...
    >
      <PAMaster name={"Preview"} 
  //  nav={`${EDIT_TEMPLATE_1}/${id}`}
   nav={fromTemplate === "Edit" ? `${EDIT_TEMPLATE_1}/${id}` : `${CREATE_TEMPLATE_1}/${id}`}
     secondName={ "Edit Template"}
    //  nav={`${CREATE_TEMPLATE_1}/${id}`}
    //  thirdName={"Create Template"} 
     />
      <Stack
        direction="row"
