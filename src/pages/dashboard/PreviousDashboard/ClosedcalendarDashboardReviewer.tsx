...
import { useGetActiveCalenderQuery, useGetClosedCalenderQuery, useGetPACalendarQuery } from '../../../service/calender/Calender';
...
const ClosedcalendarDashboardReviewer = (props: any) => {
    const {
        // valueofTab,
        setTabsActive,
    ...
    React.useEffect(()=>{
      ...
      React.useEffect(() => {
        ...
        // console.log(ccc,"ccc")
        // console.log(calendarData?.data[0],"newww")
        if(ccc != undefined){
      ...
      }, [ PAdata,activecalendardata,valueOfActiveCalender,closedCalendar])

 //For gtting active cal
// Data2 used in My team dashboard ,Rating distribution

      // const { data: data2 } = useGetPreviousAppraisalEmployeeByFilterQuery(`?select=appraisal.status,employee.employee_agree,section,sub_section,employee.employee_status,employee.employee_rating,normalizer.normalizer_rating,employee.employee_status&reviewer_code=${user?.employee_code}&calendar=${appCalId}&limit=800`)
  const { data: data1, isLoading } = useGetPreviousAppraisalEmployeeByFilterQuery(`?manager_code=${user?.employee_code}&calendar=${appCalId}&limit=800&select=appraisal.appraiser_rating,section,sub_section`);
  const SELECT_FOR_DASHBOARD = `employee_code,
  legal_full_name,
  employee_rating, service_reference_date,isSupervisor,email,function,
  ...
  reviewerIsDisabled,
  appraiser_name,
  reviewer_PA_rejected,reviewer_PA_accepted,appraiser_PA_accepted,
  normalizer_name,
  reviewer_name,
  appraisal.potential,
  manager_code,
...
normalizer_code,
  ...
  const { data: employeeData ,isLoading : isemployeeLoading} = useGetPreviousAppraisalEmployeeByFilterQuery(
      `?reviewer_code=${user?.employee_code}&calendar=${appCalId}&limit=800&select=${SELECT_FOR_DASHBOARD}`
  );
  const{data : previousappraisaldata} =useGetPreviousAppraisalEmployeeByFilterQuery('')
  console.log(previousappraisaldata,"previousappraisaldata")

  const { data: reviewerData } = useGetPreviousAppraisalEmployeeByFilterQuery(
      `?manager_code=${user?.appraiser}&calendar=${appCalId}&limit=800&select=${SELECT_FOR_DASHBOARD}`
  );
  // Data2 used in My team dashboard ,Rating distribution
  const { data: data2 } = useGetPreviousAppraisalEmployeeByFilterQuery(`?select=${SELECT_FOR_DASHBOARD}&reviewer_code=${user?.employee_code}&calendar=${appCalId}&limit=800`)
...
const { data: low_3 } = useGetPreviousAppraisalEmployeeByFilterQuery(
  `?select=overall_rating,talent_category,reviewer_PA_rejected,reviewer_PA_accepted,appraiser_PA_accepted,normalized_overallRating,employee.employee_status, service_reference_date,appraiser_rating,employee_rating,normalizer_rating,appraiser_code,reviewer_code,normalizer_code,first_name,isSupervisor,email,function,reviewer_rating,appraiser_name,work_location,reviewer_name,normalizer_name,previous_rating,manager_name,manager_position,appraisal.potential,manager_code,employee_code,legal_full_name,grade,section,sub_section,division,position_long_description,reviewer.status&reviewer_code=${user?.employee_code}&calendar=${appCalId}&appraisal.status=completed&overall_rating[gte]=${RangeLowFrom}&appraisal.potential=Low&overall_rating[lte]=${RangeLowTo}`
);

//console.log(low_3, 'useLoggedInUser')
const { data: moderate_3 } = useGetPreviousAppraisalEmployeeByFilterQuery(
  `?select=overall_rating,talent_category,reviewer_PA_rejected,reviewer_PA_accepted,appraiser_PA_accepted,normalized_overallRating,employee.employee_status, service_reference_date,appraiser_rating,employee_rating,normalizer_rating,appraiser_code,reviewer_code,normalizer_code,first_name,isSupervisor,email,function,reviewer_rating,appraisal.potential,work_location,appraiser_name,reviewer_name,normalizer_name,previous_rating,manager_name,manager_position,manager_code,employee_code,legal_full_name,grade,section,sub_section,division,position_long_description,reviewer.status&reviewer_code=${user?.employee_code}&calendar=${appCalId}&appraisal.status=completed&overall_rating[gte]=${RangeLowFrom}&appraisal.potential=Moderate&overall_rating[lte]=${RangeLowTo}`
);
...
const { data: low_5 } = useGetPreviousAppraisalEmployeeByFilterQuery(
  `?select=overall_rating,talent_category,reviewer_PA_rejected,reviewer_PA_accepted,appraiser_PA_accepted,normalized_overallRating,employee.employee_status, service_reference_date,appraiser_rating,employee_rating,normalizer_rating,appraiser_code,reviewer_code,normalizer_code,first_name,isSupervisor,email,function,reviewer_rating,appraisal.potential,work_location,appraiser_name,reviewer_name,normalizer_name,previous_rating,manager_name,manager_position,manager_code,employee_code,legal_full_name,grade,section,sub_section,division,position_long_description,reviewer.status&reviewer_code=${user?.employee_code}&calendar=${appCalId}&appraisal.status=completed&overall_rating[gte]=${RangeHighFrom}&appraisal.potential=Low&overall_rating[lte]=${RangeHighTo}`
);
const { data: moderate_5 } = useGetPreviousAppraisalEmployeeByFilterQuery(
  `?select=overall_rating,talent_category,reviewer_PA_rejected,reviewer_PA_accepted,appraiser_PA_accepted,normalized_overallRating,employee.employee_status, service_reference_date,appraiser_rating,employee_rating,normalizer_rating,appraiser_code,reviewer_code,normalizer_code,first_name,isSupervisor,email,function,reviewer_rating,appraisal.potential,appraiser_name,work_location,reviewer_name,normalizer_name,previous_rating,manager_name,manager_position,manager_code,employee_code,legal_full_name,grade,section,sub_section,division,position_long_description,reviewer.status&reviewer_code=${user?.employee_code}&calendar=${appCalId}&appraisal.status=completed&overall_rating[gte]=${RangeHighFrom}&appraisal.potential=Moderate&overall_rating[lte]=${RangeHighTo}`
);
...
console.log(subSectionFilter, "subSectionFilter");
...
useEffect(() => {
  ...
  if (
    ...
    setData1Value(data1?.data?.filter((j: any) => j?.appraisal?.status != "excepted"))
    setLow3Value(low_3)
    setLow4Value(low_4)
    setLow5Value(low_5)
  ...
  SectionFilter, subSectionFilter, employeeData, data2, low_3, low_4, low_5,
...
console.log(location,tabLocation,"tabLocation")
...
const checkAppraisalStatusInProgress = (status: string) => {
  return Data2Value?.filter((item: any) => {
    // return item?.normalizer?.normalizer_status == "reviewer-accepted-employee" && item?.appraisal?.status === status;
...
const topPerformerEmployees = () => {
  return employeeData?.data?.slice()?.sort((a: any, b: any) => b?.overall_rating - a?.overall_rating)?.slice(0, 5)
}
console.log(topPerformerEmployees(), "topPerformerEmployees")
const TopPerformersOfReviewer = employeeData1?.slice()?.filter((f: any) => f?.overall_rating >= 4 && f?.appraisal?.status == "completed")
...
export default ClosedcalendarDashboardReviewer